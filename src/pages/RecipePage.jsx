...
export default function RecipePage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.error('Failed to delete recipe:', err.response?.data?.message || err.message);
      setDeleteTargetId(null);
    }
  };
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Manage your drink recipes and ingredient compositions</p>
            </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {paginatedRecipes.map((recipe) => (
                    <tr key={recipe.id} onClick={() => setViewingItem(recipe)} className="hover:bg-slate-50 transition-colors cursor-pointer">
                      ...
                      <td className="px-5 py-4 text-right font-semibold text-slate-800">Rp {recipe.sellingPrice.toLocaleString('id-ID')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              {viewingItem && (
                <div className="space-y-4 pt-1">
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {viewingItem.ingredients.map((ing) => (
                            <tr key={ing.id}>
                              <td className="px-4 py-2.5 text-slate-700 font-medium">{ing.ingredientName}</td>
                  ...
                  <div className="flex items-center justify-between rounded-2xl border border-orange-100 bg-orange-50 px-4 py-3">
                    ...
                    <span className="text-base font-bold text-orange-600">Rp {viewingItem.sellingPrice.toLocaleString('id-ID')}</span>
                  </div>
