...
export default function PlanningModal({ open, onOpenChange, onCreated }) {
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-2xl bg-white border border-slate-200 shadow-xl rounded-4xl max-h-[90vh] overflow-y-auto">
          ...
          <form
            ...
            className="space-y-4 text-slate-800 pt-2"
          >
            <div className="space-y-1.5 min-w-0">
              <Label htmlFor="planning-name">Nama Simulasi</Label>
              <Input
                id="planning-name"
                placeholder="e.g. Rencana Produksi Minggu 1 Juli"
                {...register("name")}
            ...
            <div className="space-y-2">
                ...
                <div className="space-y-2">
                  {fields.map((field, index) => {
                    ...
                    return (
                      <div
                        key={field.id}
                        className="grid grid-cols-[1fr_90px_32px] gap-2 items-start"
                      >
                        <div>
                          <Controller
                            control={control}
                            ...
                            render={({ field: selectField }) => (
                              ...
                              >
                                <SelectTrigger
                                  aria-invalid={!!errors.menus?.[index]?.menuId}
                          ...
                          />
                          {errors.menus?.[index]?.menuId && (
                            <p className="text-xs text-destructive mt-1">
                              {errors.menus[index].menuId.message}
                            </p>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          {errors.menus?.[index]?.quantity && (
                            <p className="text-xs text-destructive mt-1">
                              {errors.menus[index].quantity.message}
                            </p>
                          )}
                        </div>
                        ...
                        {fields.length === 1 ? (
                          ...
                          <button
                            ...
                            className="size-8 flex items-center justify-center rounded-full border border-red-200 text-red-400 hover:bg-red-50 cursor-pointer"
                          >
              ...
              <button
                type="button"
                onClick={() => append({ menuId: "", quantity: "" })}
                className="flex items-center gap-1.5 text-sm font-medium text-orange-500 hover:text-orange-600 cursor-pointer mt-1"
              >
          ...
          <div className="pt-4 border-t border-slate-100 space-y-2">
            ...
            {loadingPlannings ? (
              ...
              <p className="text-sm text-slate-400 py-2">
                Belum ada simulasi yang dibuat.
              </p>
            ) : (
